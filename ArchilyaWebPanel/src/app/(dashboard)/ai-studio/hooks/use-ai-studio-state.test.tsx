...
import { AiStudioSettingsProvider } from "./use-ai-studio-settings";
...
/** Flush all pending timers AND microtasks, then re-render React */
async function deepFlush() {
  // Run pending timers
  await act(async () => {
    await vi.runOnlyPendingTimersAsync();
  });
  // Catch micro-timers scheduled by timer callbacks
  await act(async () => {
    await vi.runOnlyPendingTimersAsync();
  });
  // Final settle
...
describe("useAiStudioState", () => {
  ...
  it("clearRef clears stale job state and localStorage", async () => {
    ...
    const { result } = renderHook(() => useAiStudioState(), { wrapper: createWrapper() });

    // Flush mount restore + terminal hook (synchronous now)
    await act(async () => {
      await vi.runOnlyPendingTimersAsync();
    });
    // The terminal hook callbacks are now synchronous, so after this render
    // the result state should be set.
  ...
  // ── Result persistence: close = hide, not delete ──────────────

  it("hideResult hides result without deleting it (persists in lastResult)", async () => {
    // We test this through the facade's hideResult action.
    // First mount with a completed job so we have a result.
    seedStoredJob();
    mocks.jobState.data = createJob({
      id: storedJob.jobId,
      exists: true,
      ...
      toolId: storedJob.toolId,
      outputType: "image",
      result: { imageUrl: "https://cdn.example.com/result.png", mimeType: "image/png" },
    }) as typeof mocks.jobState.data;

    const { result } = renderHook(() => useAiStudioState(), { wrapper: createWrapper() });

    // Flush mount timers + terminal effect
    await act(async () => { await vi.runOnlyPendingTimersAsync(); });
    await act(async () => { await vi.runOnlyPendingTimersAsync(); });
    act(() => {});

    // Result should be visible after terminal callback
    expect(result.current.state.resultImage).toEqual({ src: "https://cdn.example.com/result.png", mimeType: "image/png" });

    // Hide the result
    act(() => { result.current.actions.hideResult(); });

    // Result state cleared from visible state
    expect(result.current.state.resultImage).toBeNull();
    expect(result.current.state.resultText).toBeNull();
  });

  it("restoreLastResult restores previously hidden result", async () => {
    seedStoredJob();
    mocks.jobState.data = createJob({
      id: storedJob.jobId,
      exists: true,
    ...
    // Restore it
    act(() => { result.current.actions.restoreLastResult(); });
