...
- Files in this directory are client-side service wrappers. Keep `"use client"` when browser APIs, Firebase Auth current user, canvas, File, or pdf.js are used.
- Do not import server-only modules such as `@/lib/auth/session` here.
- Fetch or callable wrappers must attach the current Firebase ID token when the corresponding API route expects `idToken`.
...
- Several callable names used here are not implemented in this repo's `functions/src/`; they may come from another deployed Firebase codebase. Preserve names and payload contracts unless the backend deployment is changed too.
