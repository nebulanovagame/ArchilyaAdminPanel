...
const VALID_PLANS = ["solo", "pro", "studio"] as const;
...
async function handler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!VALID_PLANS.includes(plan as typeof VALID_PLANS[number])) {
    return NextResponse.json(
      { error: { message: "Gecersiz plan. Gecerli degerler: solo, pro, studio", code: "invalid-argument" } },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const upstreamResponse = await fetch(`${apiBaseUrl}/admin/subscriptions/${id}/change-plan`, {
      method: "POST",
      headers: {
    ...
    console.error("Admin API /subscriptions/[id]/change-plan error:", err);
    captureApiError(err, "admin/subscriptions/[id]/change-plan");
    return NextResponse.json(
