...
async function handler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const upstreamResponse = await fetch(`${apiBaseUrl}/admin/subscriptions/${id}/refund`, {
      method: "POST",
      headers: {
    ...
    console.error("Admin API /subscriptions/[id]/refund error:", err);
    captureApiError(err, "admin/subscriptions/[id]/refund");
    return NextResponse.json(
