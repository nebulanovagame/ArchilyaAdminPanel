...
export const RenderQualityPanel: React.FC = () => {
  const [uploadedImage, setUploadedImage] = useState<string | null>(null);
  const generatedPrompt = useMemo(() => {
    if (!uploadedImage) return '';
    return 'Analyze render quality';
  }, [uploadedImage]);

  const {
    isGenerating, resultUrl, error,
    startSession, clearSession,
  } = useAiSession({ toolId: 'render-quality', enabled: true });
  ...
  useEffect(() => {
    if (resultUrl) {
      const tool = AI_TOOLS.find((t) => t.id === 'render-quality');
      void saveGeneration({
        id: `gen-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        toolId: 'render-quality',
        toolName: tool?.name || 'Render Kalite Analizi',
  ...
  useEffect(() => {
    if (error) {
      console.error('RenderQualityPanel error:', error);
    }
  }, [error]);
      ...
      {resultUrl ? (
        <>
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
            <img
              src={resultUrl}
              alt="Render kalite analiz sonucu"
        ...
        <GenerationSkeleton text="Render kalitesi analiz ediliyor..." />
      ) : (
        <>
